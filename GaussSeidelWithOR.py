...
#/////////Convergence
#while  not np.allclose(xbackup1, xbackup2, rtol=1e-8): #compare xbackup1 with xbackup2 if the direfference > 1e-7 then do another iteration 
iteration+=1
...
for i in range(0,n):
	...
	#/////////Getting x(k+1)
	xk[i]=(R[i]+xk[i])
xbackup2=copy.copy (xk)		#backup x(k+1) to avoid losing values to check convergence	
